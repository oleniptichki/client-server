...
#    f.close()
    ...
    print('failed to read PPID from file')

try:
    result=hello_client.service.killer(int(calc_id), token, int(ppid))
    print(result)
#    sleep(5)
#    result=hello_client.service.killer()
#    print result   
    ...
    if result<0:
        if (result == -2) or (result == -4):
            ...
            error="'Unexpected error: cannot kill the process'"
        elif result == -7:
            error="'Unexpected error: cannot remove files'"
        elif result == -8:
            error="'Unexpected error: cannot kill the process'"
        else:
